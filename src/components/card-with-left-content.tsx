            ...
            onMouseOut={handleMouseOut}>
                <div className='shadow-3xl'>
                    <div className="relative">
                        {data?.image && 
                        <div><img src={data?.image} alt="card-image" /></div>
                        }
                        {data?.tag && 
                        <div className="absolute top-0 px-5 py-2 text-white text-xs font-bold bg-chilly" >{data?.tag}</div>
                        }
                    </div>
                    <div className='mt-4 ml-2.5 mb-7 mr-5'>
                        <div className='flex justify-between item-center'>
                            <div>
                                {data?.title && 
                                <div className='text-xl font-bold text-pepermint' >{data?.title}</div>
                                }
                                {(data?. duration && data?.time) && 
                                <div className='text-base font-bold'>{data?.duration} : {data?.time}</div>
                                }
                            </div>
                            {isHovering &&
                            <div onClick={()=>setAddItem(!addItem)}>{addItem ? 
                            <>
                            <AddedIcon/>
                            <div className='text-maroon text-xs font-bold text-center'>{CONSTANT?.ADDED}</div></> 
                            :<>
                            <AddIcon sx={{width:"35px"}}/>
                            <div className='text-offWhite text-xs font-bold text-center'>{CONSTANT?.ADD}</div> 
                            </>}
                        </div>
                            }
                        </div>
                        ...
                        {data?.price && 
                            <div className='text-base text-ash' style={{textDecoration: data?.offer ? "line-through" : "unset" }}>{data?.price}</div>
                        }
                        {data?.offerPrice && 
                            <div className='text-base font-style: italic text-red-600 font-bold'>{data?.offerPrice}</div>
                        }
                        </div>
                        {data?.instituteName && 
                        <div className='w-[69px] h-[10px] mt-[8px] mb-[11px]'><img src={IMAGES?.KHAN_ACADEMY} alt="institue-name" /></div>
                        }
